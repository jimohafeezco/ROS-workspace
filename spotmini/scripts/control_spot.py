...
class spotminiJointMover(object):

    def __init__(self):
        rospy.init_node('jointmover_demo', anonymous=True)
        rospy.loginfo("spotmini JointMover Initialising...")
        ...
        self.pub_upperlegM3_joint_position = rospy.Publisher(
            ...
            Float64,
            queue_size=1)
        self.pub_upperlegM4_joint_position = rospy.Publisher(
            '/spotmini/head_upperlegM3_joint_position_controller/command',
            Float64,
            queue_size=1)
        ...
        self.pub_lowerlegM3_joint_position = rospy.Publisher(
            ...
            Float64,
            queue_size=1)
        self.pub_lowerlegM4_joint_position = rospy.Publisher(
            '/spotmini/upperlegM3_lowerlegM3_joint_position_controller/command',
            Float64,
            queue_size=1)
            
        joint_states_topic_name = "/spotmini/joint_states"
        rospy.Subscriber(joint_states_topic_name, JointState, self.spotmini_joints_callback)
        spotmini_joints_data = None
        rate = rospy.Rate(2)
        while spotmini_joints_data is None:
            try:
                spotmini_joints_data = rospy.wait_for_message(joint_states_topic_name, JointState, timeout=5)
            except:
                rospy.logwarn("Time out " + str(joint_states_topic_name))
                pass
            rate.sleep()

        self.spotmini_joint_dictionary = dict(zip(spotmini_joints_data.name, spotmini_joints_data.position))

    def move_spotmini_all_joints(self, upperlegM1_angle, upperlegM2_angle, upperlegM3_angle, upperlegM4_angle, lowerlegM1_value, lowerlegM2_value ,lowerlegM3_value, lowerlegM4_value):
        upperlegM1 = Float64()
        ...
        upperlegM4 = Float64()
        upperlegM4.data = upperlegM3_angle
        ...
        lowerlegM4 = Float64()
        lowerlegM4.data = lowerlegM3_value
        ...
        self.pub_upperlegM4_joint_position.publish(upperlegM3)
        ...
        self.pub_lowerlegM4_joint_position.publish(lowerlegM3)


    def spotmini_joints_callback(self, msg):
        """
        ...
        :return:
        """
        self.spotmini_joint_dictionary = dict(zip(msg.name, msg.position))
    ...
    def spotmini_check_continuous_joint_value(self, joint_name, value, error=0.1):
        """
        ...
        :return:
        """
        joint_reading = self.spotmini_joint_dictionary.get(joint_name)
        if not joint_reading:
            print("self.spotmini_joint_dictionary="+str(self.spotmini_joint_dictionary))
    ...
    def spotmini_movement_look(self, upperlegM1_angle, upperlegM2_angle, upperlegM3_angle, upperlegM4_angle, lowerlegM1_value, lowerlegM2_value ,lowerlegM3_value, lowerlegM4_value):
        """
        Move:
        ...
        while not (similar_upperlegM1 and similar_upperlegM2 and similar_upperlegM3 and similar_lowerlegM1 and similar_lowerlegM2 and similar_lowerlegM3):
            self.move_spotmini_all_joints(position_upperlegM1,
                                       position_upperlegM2,
                                       position_upperlegM3,
                                       position_upperlegM4,
                                       position_lowerlegM1,
                                       ...
                                       position_lowerlegM3,
                                       position_lowerlegM4)
            similar_upperlegM1 = self.spotmini_check_continuous_joint_value(joint_name="head_upperlegM1_joint",
                                                                         value=position_upperlegM1)
            similar_upperlegM2 = self.spotmini_check_continuous_joint_value(joint_name="head_upperlegM2_joint",
                                                                         value=position_upperlegM2)
            similar_upperlegM3 = self.spotmini_check_continuous_joint_value(joint_name="head_upperlegM3_joint",
                                                                         value=position_upperlegM3)
            similar_upperlegM4 = self.spotmini_check_continuous_joint_value(joint_name="head_upperlegM4_joint",
                                                                         value=position_upperlegM4)
            similar_lowerlegM1 = self.spotmini_check_continuous_joint_value(joint_name="upperlegM1_lowerlegM1_joint",
                                                                         value=position_lowerlegM1)
            similar_lowerlegM2 = self.spotmini_check_continuous_joint_value(joint_name="upperlegM2_lowerlegM2_joint",
                                                                         value=position_lowerlegM2)
            similar_lowerlegM3 = self.spotmini_check_continuous_joint_value(joint_name="upperlegM3_lowerlegM3_joint",
                                                                         value=position_lowerlegM3)
            similar_lowerlegM4 = self.spotmini_check_continuous_joint_value(joint_name="upperlegM4_lowerlegM4_joint",
                                                                         value=position_lowerlegM4)

            rate.sleep()

    def spotmini_init_pos_sequence(self):
        """
        UPPER limits lower="-1.55" upper="0.0"
        LOWER limits lower="-2.9" upper="1.5708"
        :return:
        """
        upperlegM1_angle = -0.2
        upperlegM2_angle = -0.2
        upperlegM3_angle = -0
        upperlegM4_angle = -0
        lowerlegM1_angle = -0.2
        lowerlegM2_angle = -0.2
        lowerlegM3_angle = 0.0
        lowerlegM4_angle = 0.0

        self.spotmini_movement_look(upperlegM1_angle,
                                 upperlegM2_angle,
                                 upperlegM3_angle,
                                 upperlegM4_angle,
                                 lowerlegM1_angle,
    ...
    def spotmini_hop(self, num_hops=15):
        """
        UPPER limits lower="-1.55" upper="0.0"
        LOWER limits lower="-2.9" upper="1.5708"
        :return:
        ...
        #self.spotmini_init_pos_sequence()
        for repetitions in range(num_hops):
            self.spotmini_movement_look(upperlegM_angle,
                                     upperlegM_angle,
                                     upperlegM_angle,
                                     upperlegM_angle,
                                     lowerlegM_angle,
    ...
    def spotmini_moverandomly(self):
        """
        UPPER limits lower="-1.55" upper="0.0"
        LOWER limits lower="-2.9" upper="1.5708"
        :return:
    ...
    def movement_random_loop(self):
        ...
        :return:
        """
        rospy.loginfo("Start Moving spotmini...")
        while not rospy.is_shutdown():
            self.spotmini_init_pos_sequence()
            # self.spotmini_moverandomly()
            self.spotmini_hop()

if __name__ == "__main__":
    spotmini_jointmover_object = spotminiJointMover()
    spotmini_jointmover_object.movement_random_loop()
