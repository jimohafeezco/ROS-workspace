...
/spot/head_upperlegM1_joint_position_controller/command
/spot/head_upperlegM2_joint_position_controller/command
/spot/head_upperlegM3_joint_position_controller/command
/spot/upperlegM1_lowerlegM1_joint_position_controller/command
/spot/upperlegM2_lowerlegM2_joint_position_controller/command
/spot/upperlegM3_lowerlegM3_joint_position_controller/command
"""

class spotJointMover(object):

    def __init__(self):
        rospy.init_node('jointmover_demo', anonymous=True)
        rospy.loginfo("spot JointMover Initialising...")
        ...
        self.pub_lowerlegM3_joint_position = rospy.Publisher(
            ...
            Float64,
            queue_size=1)
        joint_states_topic_name = "/spot/joint_states"
        rospy.Subscriber(joint_states_topic_name, JointState, self.spot_joints_callback)
        spot_joints_data = None
        rate = rospy.Rate(2)
        while spot_joints_data is None:
            try:
                spot_joints_data = rospy.wait_for_message(joint_states_topic_name, JointState, timeout=5)
            except:
                rospy.logwarn("Time out " + str(joint_states_topic_name))
                pass
            rate.sleep()

        self.spot_joint_dictionary = dict(zip(spot_joints_data.name, spot_joints_data.position))

    def move_spot_all_joints(self, upperlegM1_angle, upperlegM2_angle, upperlegM3_angle, lowerlegM1_value, lowerlegM2_value ,lowerlegM3_value):
        upperlegM1 = Float64()
    ...
    def spot_joints_callback(self, msg):
        """
        ...
        :return:
        """
        self.spot_joint_dictionary = dict(zip(msg.name, msg.position))
    ...
    def spot_check_continuous_joint_value(self, joint_name, value, error=0.1):
        """
        ...
        :return:
        """
        joint_reading = self.spot_joint_dictionary.get(joint_name)
        if not joint_reading:
            print("self.spot_joint_dictionary="+str(self.spot_joint_dictionary))
    ...
    def spot_movement_look(self, upperlegM1_angle, upperlegM2_angle, upperlegM3_angle, lowerlegM1_value, lowerlegM2_value ,lowerlegM3_value):
        """
        Move:
        ...
        while not (similar_upperlegM1 and similar_upperlegM2 and similar_upperlegM3 and similar_lowerlegM1 and similar_lowerlegM2 and similar_lowerlegM3):
            self.move_spot_all_joints(position_upperlegM1,
                                       position_upperlegM2,
                                       position_upperlegM3,
                                       position_lowerlegM1,
                                       position_lowerlegM2,
                                       position_lowerlegM3)
            similar_upperlegM1 = self.spot_check_continuous_joint_value(joint_name="head_upperlegM1_joint",
                                                                         value=position_upperlegM1)
            similar_upperlegM2 = self.spot_check_continuous_joint_value(joint_name="head_upperlegM2_joint",
                                                                         value=position_upperlegM2)
            similar_upperlegM3 = self.spot_check_continuous_joint_value(joint_name="head_upperlegM3_joint",
                                                                         value=position_upperlegM3)
            similar_lowerlegM1 = self.spot_check_continuous_joint_value(joint_name="upperlegM1_lowerlegM1_joint",
                                                                         value=position_lowerlegM1)
            similar_lowerlegM2 = self.spot_check_continuous_joint_value(joint_name="upperlegM2_lowerlegM2_joint",
                                                                         value=position_lowerlegM2)
            similar_lowerlegM3 = self.spot_check_continuous_joint_value(joint_name="upperlegM3_lowerlegM3_joint",
                                                                         value=position_lowerlegM3)

            rate.sleep()

    def spot_init_pos_sequence(self):
        """
        ...
        self.spot_movement_look(upperlegM1_angle,
                                 upperlegM2_angle,
                                 upperlegM3_angle,
                                 lowerlegM1_angle,
                                 lowerlegM2_angle,
    ...
    def spot_hop(self, num_hops=15):
        ...
        #self.spot_init_pos_sequence()
        for repetitions in range(num_hops):
            self.spot_movement_look(upperlegM_angle,
                                     upperlegM_angle,
                                     upperlegM_angle,
                                     lowerlegM_angle,
                                     lowerlegM_angle,
    ...
    def movement_random_loop(self):
        ...
        :return:
        """
        rospy.loginfo("Start Moving spot...")
        while not rospy.is_shutdown():
            self.spot_init_pos_sequence()
            #self.spot_moverandomly()
            self.spot_hop()

if __name__ == "__main__":
    spot_jointmover_object = spotJointMover()
    spot_jointmover_object.movement_random_loop()
